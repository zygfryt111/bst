...
/*double srednia(struct wezel *t);
double ave(struct wezel *t);
int suma(struct wezel *t);
int licz(struct wezel *t);
void suma_licz(struct wezel *t, int *suma, int *licz);
*/
...
/*int suma(struct wezel *t){
    if(t == NULL)
        return 0;
    return t->value + suma(t->r) + suma(t->l);
}
int licz(struct wezel *t){
    if(t == NULL)
        return 0;
    return 1 + licz(t->r) + licz(t->l);
}
double ave(struct wezel *t){
    return (double)suma(t) / licz(t);
}
void suma_licz(struct wezel *t, int *suma, int *licz){
    if(t != NULL){
        *suma += t->value;
        ++*licz;
        suma_licz(t->l, suma, licz);
        suma_licz(t->r, suma, licz);
    }
}
double srednia(struct wezel *t){
    int suma=0, licz=0;
    suma_licz(t, &suma, &licz);
    return (double)suma / licz;
}   
*/
//funkcje wykomentowane z powodu zlego podejscia do problemu
