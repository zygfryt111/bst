...
int nazwa(char *nazwa_pliku,char *rozszerzenie, char *nazwa_pliku_z_nowym_rozszerzeniem)
{
    int i=0;
    int n=0;
    for(i=0;i<35;i++)
...
int obliczPomMZI(struct element_zrodla model[],struct element_zrodla model_pelny[256])
{
    int n=0,i;
    for(i=0;i<256;i++)
    {
...
int oblicz_MZI(char *nazwa_pliku, char *nazwa_pliku_model, char *nazwa_pliku_wynik, struct element_zrodla model[])
{
    int liczba_znakow=0;
    int n=0,k=0,j=0,i=0;
    ...
    else
    {
        ...
        licznik_symboli=obliczPomMZI(model,model_pelny);
...
int main(int argc, char *argv[])
{
    ...
    printf("Kodowanie / dekodowanie Huffmana\n");
    wybor=1;
    while(wybor==1 || wybor==2)
    {
        printf("1.Kompresja\n");
        printf("2.Dekompresja (nie dziala, poki co)\n");
        scanf("%d",&wybor);
        if(wybor==1)
        {
            printf("Okresl plik do kompresji: \n");
            scanf("%s",&nazwa_pliku);
            nazwa(nazwa_pliku,rozszerzenie_modelu,nazwa_pliku_modelu);
            printf("MZI: %s\n",nazwa_pliku_modelu);
            nazwa(nazwa_pliku,rozszerzenie_sort,nazwa_pliku_sort);
            printf("Posortowane MZI: %s \n",nazwa_pliku_sort);
            nazwa(nazwa_pliku,rozszerzenie_modelu,nazwa_pliku_modelu);
            printf("Dodatkowe MZI: %s \n",nazwa_pliku_mod_modelu);
            nazwa(nazwa_pliku,rozszerzenie_drzewo,nazwa_pliku_drzewo);
            printf("Drzewo kodowania: %s \n",nazwa_pliku_drzewo);
            oblicz_MZI(nazwa_pliku,nazwa_pliku_modelu,nazwa_pliku_drzewo,model);
